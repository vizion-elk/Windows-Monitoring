...
if($principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Set-Location -Path 'c:\windows-monitoring-7.7.0\server\winlogbeat'
    Set-ExecutionPolicy Unrestricted
    "Winlogbeat Execution policy set - Success"

    Set-Location -Path 'c:\windows-monitoring-7.7.0\server\auditbeat'
    Set-ExecutionPolicy Unrestricted
    "Auditbeat Execution policy set - Success"

    Set-Location -Path 'c:\windows-monitoring-7.7.0\server\metricbeat'
    Set-ExecutionPolicy Unrestricted
